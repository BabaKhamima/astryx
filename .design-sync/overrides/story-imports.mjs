...
// Story files in this repo (28+ of them) call `@stylexjs/stylex`'s
// `stylex.create()` directly at module scope for demo-only wrapper layout
// (separate from the actual component styles, which ship precompiled in the
// bundle and are unaffected). The REAL @stylexjs/stylex package's `create()`
// deliberately THROWS at runtime unless the call was compiled away by
// babel-plugin-stylex — which this converter's plain-esbuild preview
// compile never runs. Left unhandled, every story that calls it crashes at
// import time, producing a root-empty preview — bundling the real package
// (the default policyRedirect/dsShim behavior for node_modules) does not
// help, since the throw is intentional regardless of how it's resolved.
//
// v1 fix (superseded below): shim to a TRUE no-op (create passes through,
// props() returns {}). This stopped the crash but turned out to silently
// DROP every story's own demo-only layout styling too (backgrounds, widths,
// borders, gaps on wrapper elements) — confirmed independently by two
// fan-out batches (Card/Center/Carousel; ContextMenu/Collapsible). A no-op
// is a worse failure mode than a crash here: it fails quietly.
//
// v2 fix: `create()` still passes style objects through unchanged (real
// StyleX would key them into atomic classes; we don't compile, so we keep
// the raw CSS-like object as the "value" for that style key). `props()` now
// actually APPLIES those objects as an inline `style`, best-effort. This
// recovers plain flat declarations (display/gap/width/backgroundColor/...)
// exactly as authored. Known gaps (acceptable — these are demo-only
// wrapper styles, not the shipped component's own styles, which are
// unaffected either way): pseudo-class keys (`:hover`) and nested
// media-query objects aren't real CSS when spread into inline `style` —
// React silently ignores keys it doesn't recognize as camelCase CSS
// properties, so this degrades to "no hover effect" rather than a crash.
const STYLEX_STUB_SRC = `
export function create(styles) { return styles; }
export function props(...args) {
  const style = {};
  for (const s of args) {
    if (!s || typeof s !== 'object') continue;
    for (const [k, v] of Object.entries(s)) {
      // Skip pseudo-class / at-rule keys (":hover", "@media ...") and nested
      // objects generally — only flat CSS-like declarations apply cleanly
      // to an inline style object.
...
export function defineVars(vars) { return vars; }
export function createTheme(vars) { return vars; }
export function keyframes(k) { return k; }
export function firstThatWorks(...vals) { return vals[0]; }
export default { create, props, defineVars, createTheme, keyframes, firstThatWorks };
`;

function stylexStubPlugin() {
  return {
    name: 'ds-stylex-stub',
    setup(b) {
      b.onResolve({ filter: /^@stylexjs\/stylex(\/.*)?$/ }, (a) => ({ path: a.path, namespace: 'ds-stylex-stub' }));
      b.onLoad({ filter: /.*/, namespace: 'ds-stylex-stub' }, () => ({ contents: STYLEX_STUB_SRC, loader: 'js' }));
    },
  };
}
...
export function storyImportPlugins({ PKG, GLOBAL, extraEntries = [], exported, cfg, pkgDir }) {
  ...
  return {
    plugins: [stylexStubPlugin(), dsShim, storybookStubPlugin(), consoleStub, policyRedirect],
